...
def usdt(c):
    ...
    msg = bot.send_message(c.message.chat.id,"Send USDT-BEP20 address (starts with 0x)")
...
def ask_address(m):
    ...
    if not address.startswith("0x"):
        msg = bot.send_message(m.chat.id,"❌ Invalid address. Please enter a valid USDT-BEP20 address starting with 0x")
        bot.register_next_step_handler(msg, ask_address)
        return
    users[uid]["temp_address"] = address
    save("users.json", users)
    msg = bot.send_message(m.chat.id,f"Your balance is ${users[uid]['balance']:.2f}\nEnter amount to withdraw:")
...
def ask_amount(m):
    ...
    try:
        ...
        msg = bot.send_message(m.chat.id,"❌ Invalid amount. Enter a number:")
        bot.register_next_step_handler(msg, ask_amount)
        return
    if amount > users[uid]["balance"] or amount < 0.5:
        msg = bot.send_message(m.chat.id,f"❌ Invalid amount. Your balance: ${users[uid]['balance']:.2f}. Minimum withdrawal: $0.5")
        bot.register_next_step_handler(msg, ask_amount)
        return
    ...
    withdraws.append({
        ...
        "user": uid,
        "amount": amount,
        "address": users[uid]["temp_address"],
...
def stats(c):
    ...
    bot.send_message(c.message.chat.id, f"Users: {total}\nTotal Balance: ${totalbal}")
...
def download(chat, url):
    try:
        ...
        bot.send_message(chat,f"Download error: {e}")
