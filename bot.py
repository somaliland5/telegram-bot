        ...
        if ref_code:
            for uid in users:
                if users[uid]["ref_code"] == ref_code:
                    ...
                    bot.send_message(int(uid), "🎉 New referral joined\n💰 You earned $0.5")
    save_users(users)
    bot.send_message(message.chat.id, "👋 Welcome to Downloader & Earn Bot")
...
def get_id(message):
    users = load_users()
    uid = str(message.from_user.id)
    bot.send_message(message.chat.id, f"🆔 Your Bot ID: {users[uid]['bot_id']}")
...
def withdraw_amount_step(message):
    msg = bot.send_message(message.chat.id, "Enter withdrawal amount (Minimum $1):")
...
def withdraw_amount_process(message):
    ...
    if amount > users[uid]["balance"]:
        ...
        return
    msg = bot.send_message(message.chat.id, "Enter your USDT-BEP20 address:")
    bot.register_next_step_handler(msg, withdraw_address_process, amount)

def withdraw_address_process(message, amount):
    users = load_users()
    uid = str(message.from_user.id)
...
def callbacks(call):
    ...
    users = load_users()
    if data[0] == "confirm":
        uid = data[1]; amount = data[2]
        bot.send_message(uid, f"✅ Withdrawal Confirmed\n💰 ${amount}")
    if data[0] == "reject":
        uid = data[1]; amount = float(data[2])
        users[uid]["balance"] += amount
        save_users(users)
